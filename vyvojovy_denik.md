...
### 10.5 Bezpečné uzamčení administrační zóny přístupovým heslem (GDPR Compliance)
Vzhledem k tomu, že systém pracuje s reálnými osobními údaji uchazečů (jména, e-maily, telefonní čísla, současné základní školy a případné specifické vzdělávací potřeby), je z hlediska legislativy o ochraně osobních údajů (GDPR) nepřípustné, aby administrační panel zůstal veřejně přístupný komukoliv na internetu. V rámci finálního nasazení bylo do systému implementováno komplexní a robustní zabezpečení přístupu heslem:

1. **Zabezpečení REST API (Server-Side)**: Všechny backendové endpointy v [app.py](file:///c:/Users/kotlanova/.gemini/antigravity-ide/scratch/open-doors-registration/app.py), které manipulují s osobními údaji nebo stavem databáze (`/api/visitors` pro zobrazení tabulky a CSV export, `/api/checkin` pro odbavování, `/api/clear` pro mazání a `/api/reset` pro demo data), byly chráněny pomocí pomocné funkce `check_admin_auth()`. Server vyžaduje předání platného přístupového hesla v bezpečné vlastní HTTP hlavičce `X-Admin-Password`. Pokud heslo v hlavičce chybí nebo je nesprávné, server okamžitě odpovídá stavovým kódem `401 Unauthorized`. To znemožňuje jakékoli zneužití např. přímým dotazováním na API přes vývojářskou konzoli. Výjimkou zůstává pouze endpoint `/api/stats`, který neobsahuje žádné osobní údaje a slouží k veřejnému zobrazování obsazenosti časových slotů na registrační stránce.
2. **Elegantní přihlašovací rozhraní (Client-Side SPA Router)**: Na frontendu byla vytvořena nová vizuálně působivá a plně responzivní přihlašovací sekce (`#admin-login-view`). Pokud uživatel klikne v navigaci na záložku *„📊 Administrace“*, směrovač v [app.js](file:///c:/Users/kotlanova/.gemini/antigravity-ide/scratch/open-doors-registration/static/js/app.js) nejprve zkontroluje stav jeho relace v `sessionStorage`. Pokud není přihlášen, nezobrazí dashboard administrace, ale přesměruje jej na přihlašovací kartu.
3. **Plynulé a bezpečné ovládání**:
   - Přihlašovací karta je navržena v duchu Citroën glassmorphic stylu, obsahuje bezpečné maskované pole pro zadávání hesla s interaktivním přepínačem viditelnosti (ikona oka `👁️` / `🙈`) a vizuální indikaci ověřování (asynchronní načítání tlačítka).
